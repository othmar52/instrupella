...
export const useMidiStore = defineStore({
  ...
  state: () => ({
    ...
    midiInputClockPriority: [RC202, DIGITAKT, USBMIDICABLE, CONTROLHUB],
  ...
  getters: {
    ...
    getExternalClockTempo() {
      return parseFloat(
        this.clock.overrideTempo > 0
          ? this.clock.overrideTempo
          : this.clock.tempo
      )
    },
    ...
    getOverrideTempo() {
      return this.clock.overrideTempo
    },
  ...
  actions: {
    ...
    setOverrideTempo(value) {
      this.clock.overrideTempo = value
    },
    setGuiAlerts(value) {
      ...
      this.midiInputClock.addListener('midimessage', midiEvent => {
        switch (midiEvent.message.type) {
          ...
          case 'clock':
            ...
            if (this.clock.overrideTempo > 0) {
              newTempo = this.clock.overrideTempo
            }

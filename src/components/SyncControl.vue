  ...
  <div class="sync-control d-flex">
    <div class="sync-control-button-group d-flex flex-column">
      ...
      <Button
        ...
        :permaClasses="`${buttonClasses}`"
        :activeClass="deck.syncResolution === 0 ? 'btn-danger' : ''"
        @click="loopSyncResolution"
      />
    </div>
  </div>
</template>
...
const loopSyncResolution = (factor) => {
  storage.loopSyncResolution(props.deck.index)
  storage.syncTempoToExternalClock()
  midistorage.resyncDeck(props.deck.index)
}
...
const syncResolutionLabel = computed(() => {
  ...
  if (props.deck.syncResolution === 0) {
    return 'none'
  }
  return '1 bar'
})
